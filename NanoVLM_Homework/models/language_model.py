...
class LanguageModelBlock(nn.Module):
    ...
    def forward_kv(self, x, cos, sin, past_key_value=None):
        ...
        x, present_key_value = self.attn.forward_kv(x,cos = cos,
                                                    sin = sin,
                                                    past_key_value=past_key_value) # call self.attn.forward_kv
...
class LanguageModel(nn.Module):
    ...
    def forward_kv(self, x, past_key_values=None, attention_mask=None):
        ...
        if past_key_values is None:
            past_length = 0   # past length is 0 if past_key_values is None, otherwise get the length from past_key_values
        else:
        ...
        for i, block in enumerate(self.blocks):
            if past_key_values is not None:
                past_kv =  past_key_values[i] # fetch this layer's cache entry (or None on first call)
            else:
                past_kv = None
