...
class VisionLanguageModel(nn.Module):
    ...
    def forward(self, input_ids, image, attention_mask=None, targets=None):
        ...
        if attention_mask is not None:
            B, num_image_tokens, _ = image_embeds.size()
            image_attention = torch.ones(size= (B,num_image_tokens),
                                         device = attention_mask.device,
                                         dtype = attention_mask.dtype) # define attention mask for image
        ...
        if targets is not None:
            ...
            logits = logits[:, -text_embeds.size(1):, :]
    ...
    def generate(self, input_ids, image, attention_mask=None, max_new_tokens=5):
        ...
        if attention_mask is not None:
            image_attention_mask = torch.ones(size=(batch_size, img_seq_len),
                                               device=attention_mask.device,
        ...
        for i in range(max_new_tokens):
            ...
            model_out = model_out = self.decoder(outputs, attention_mask)  # (i)
            ...
            generated_tokens[:, i] = next_token.squeeze()  # fill in the generated next token in generated_tokens at ith position
    ...
    def generate_with_kv_cache(self, input_ids, image, attention_mask=None, max_new_tokens=5):
        ...
        image_embeds = self.MP(image_embeds)
        text_embeds =  self.decoder.token_embedding(input_ids)
        combined_embd = torch.cat([image_embeds, text_embeds], dim=1)
        ...
        generated_tokens = torch.zeros((batch_size, max_new_tokens), device=input_ids.device, dtype=input_ids.dtype)
        generated_tokens[:, 0] = next_token.squeeze() # fill in the generated next token in generated_tokens at 0th position
