...
class FourM(nn.Module):
    ...
    def forward_encoder(
        ...
        x = self.enc_tok_emb(enc_input_tokens) # It project each descreate token into the embedding dimension
    ...
    def generate_one_modality_roar(
        ...
        random_mask = torch.rand(n_tokens_target, device=device)
        idx = torch.argsort(random_mask)
        ...
        dec_input_positions_list = [subset.unsqueeze(0) for subset in torch.split(idx, schedule)]
        ...
        for step, k in enumerate(schedule):
            ...
            predicted_logits = predicted_logits[0] # Select the 0-th element 
