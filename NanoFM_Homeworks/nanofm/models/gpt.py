...
class GPT(nn.Module):
    ...
    def forward_model(self, x: torch.LongTensor) -> torch.Tensor:
        ...
        causal_mask = torch.tril(torch.ones(1, L, L, dtype=torch.bool)) 
    ...
    def compute_ce_loss(self, logits: torch.Tensor, target_seq: torch.LongTensor, padding_idx: int = -100) -> torch.Tensor:
        ...
        loss = nn.functional.cross_entropy(logits.reshape(-1, logits.size(-1)), target_seq.reshape(-1), ignore_index=padding_idx)
    ...
    def generate(
        ...
        for _ in range(self.max_seq_len - len(context)):
            ...
            next_token = sample_tokens(logits[:, -1, :], temp=temp, top_p=top_p, top_k=top_k)
